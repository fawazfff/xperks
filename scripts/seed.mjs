...
if (!key || !/^0x[0-9a-fA-F]{64}$/.test(key) || !contract || !/^0x[0-9a-fA-F]{40}$/.test(contract)) throw new Error("Set DEPLOYER_PRIVATE_KEY and XPERKS_CONTRACT in the shell.");
const chain = defineChain({ id: 1952, name: "X Layer Testnet", nativeCurrency: { name: "OKB", symbol: "OKB", decimals: 18 }, rpcUrls: { default: { http: ["https://testrpc.xlayer.tech/terigon"] } } });
...
await send("createBenefit", ["Tesla Holder Pack", "A thank-you for people holding demo Tesla shares.", "Welcome to the Tesla Holder Pack. Your ownership has been verified on X Layer testnet.", parseUnits("0.01", 18)]);
const id = await publicClient.readContract({ address: contract, abi, functionName: "benefitCount" });
await send("claimDemoShares", []);
await send("claimBenefit", [id]);
if (!await publicClient.readContract({ address: contract, abi, functionName: "hasClaimed", args: [id, account.address] })) throw new Error("Claim verification failed");
